...
const AdminLayout = () => {
  ...
  const menuItems = [
    ...
    { title: 'Website Content', path: '/admin/website', icon: <FiGlobe /> },
  ...
  return (
    <div className="flex min-h-screen bg-[#F3F3F3] font-montserrat overflow-x-hidden">
      {/* Mobile Overlay */}
      <AnimatePresence>
        {isSidebarOpen && (
          <motion.div
      ...
      >
        <div className={`h-14 flex items-center border-b border-white/10 shrink-0 mt-2 ${
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-[#FFB400] flex items-center justify-center p-1.5 shrink-0">
            ...
            <div className={`transition-all duration-300 overflow-hidden whitespace-nowrap ${isSidebarOpen ? 'opacity-100 max-w-[200px]' : 'opacity-0 max-w-0 hidden'}`}>
              <h1 className="font-normal text-[15px] tracking-wide leading-tight uppercase" style={{ fontFamily: "'Oswald', sans-serif" }}>Build My Destiny</h1>
              <p className="text-[9px] uppercase tracking-[0.2em] text-[#FFB400]">Admin Panel</p>
            </div>
          </div>
        </div>
          ...
          {menuItems.map((item) => (
            <React.Fragment key={item.title}>
              {item.subItems ? (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`shrink-0 transition-all duration-300 ${location.pathname === item.path ? 'text-black' : ''}`}>
                      {React.cloneElement(item.icon, { size: 18 })}
                    </div>
      ...
      <div className={`flex-1 flex flex-col min-w-0 transition-all duration-500 ${isSidebarOpen ? 'ml-64' : 'ml-16'}`}>
        ...
        <header className="sticky top-0 h-14 bg-white border-b border-gray-100 flex items-center justify-between px-6 z-40 shadow-sm shrink-0">
          <div className="flex items-center gap-4 flex-1">
            ...
            <h2 className="block text-[13px] sm:text-[15px] font-oswald font-bold text-black shrink-0 tracking-widest uppercase">Admin Panel</h2>
          ...
          <div className="flex items-center gap-3 sm:gap-4 shrink-0">
            ...
            <div className="relative">
              ...
              >
                <FiBell size={20} className="text-[#111111]" />
                {unreadCount > 0 && (
                ...
                {isNotificationsOpen && (
                  ...
                  >
                    <div className="flex justify-between items-center px-4 py-3 border-b border-gray-100">
                      ...
                      {unreadCount > 0 && (
                        <button 
                          ...
                          className="text-xs text-[#FFB400] font-semibold hover:underline"
                        >
                          Mark all read
                        </button>
                      )}
                    ...
                    <Link 
                      ...
                      className="block w-full text-center py-2 text-xs font-bold text-black bg-[#FFB400] hover:bg-[#ffc433] rounded-b-xl"
                    >
            ...
            <div className="relative">
              ...
              >
                ...
                <img src={`https://ui-avatars.com/api/?name=${user?.name || 'Admin+User'}&background=111111&color=FFB400`} alt="Admin" className="w-8 h-8 rounded-full border border-gray-100 shadow-sm" />
                ...
                {isProfileOpen && (
                  ...
                  >
                    <div className="px-4 py-3 border-b border-gray-50 mb-2">
                      ...
                      <p className="text-[11px] text-gray-500 font-medium truncate">{user?.email || 'admin@buildmydestiny.com'}</p>
                    </div>
        ...
        {/* Content Container */}
        <main className="bg-[#F3F3F3] relative min-h-[calc(100vh-56px)] overflow-x-hidden p-4 md:p-5 lg:p-6">
          <Outlet />
        </main>
      </div>
    </div>
